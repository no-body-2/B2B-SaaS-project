  ...
  if (!activeNano) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-slate-950">
        ...
        <h3 className="text-slate-350 font-bold text-base">선택된 문서가 없습니다</h3>
        <p className="text-slate-500 text-xs mt-1">좌측 사이드바에서 편집하거나 조회할 협업 문서를 골라보세요.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-950">
        ...
        <div className="flex items-center gap-3">
          {isEditMode ? (
            <input
              ...
              className="px-3 py-1.5 border border-slate-800 rounded-lg text-sm font-bold bg-slate-950 text-slate-100 focus:outline-none focus:ring-1 focus:ring-violet-500"
            />
          ) : (
          ...
          {isEditMode ? (
            <>
              <button
                ...
                className="px-3.5 py-1.5 border border-slate-800 text-slate-300 hover:bg-slate-800 rounded-lg text-xs font-semibold transition cursor-pointer bg-transparent"
              >
                취소
              </button>
              <button
                ...
                disabled={saving}
                className="px-3.5 py-1.5 bg-violet-600 hover:bg-violet-700 disabled:bg-violet-600/60 text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer border border-transparent"
              >
            ...
            <>
              <button
                ...
                className="px-3.5 py-1.5 border border-slate-800 text-slate-300 hover:bg-slate-800 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer bg-transparent"
              >
              ...
              {isOwner && (
                <button
                  ...
                  className="p-2 text-slate-400 hover:text-red-400 rounded-lg hover:bg-slate-800 transition cursor-pointer border border-transparent bg-transparent"
                  title="문서 삭제"
                >
      ...
      <div className="flex-1 overflow-y-auto p-8 max-w-4xl w-full mx-auto flex flex-col gap-6">
        ...
        {/* 문서 정보 뱃지 메타데이터 */}
        <div className="flex gap-4 text-xs text-slate-400 border-b border-slate-900 pb-4">
        ...
        {isEditMode ? (
          <textarea
            ...
            className="flex-1 w-full min-h-[450px] p-6 border border-slate-850 rounded-xl text-sm font-mono bg-slate-900 text-slate-200 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 resize-none leading-relaxed shadow-inner"
          />
        ) : (
          <div className="flex-1 bg-slate-900 border border-slate-850 rounded-xl p-8 min-h-[450px] text-sm text-slate-200 whitespace-pre-wrap leading-relaxed shadow-sm">
            {activeNano.content || (
      ...
      {/* 3. 일반 MEMBER용 문서 수정 결재 상신 모달 */}
      {isApprModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex justify-center items-center p-4">
          <div className="w-full max-w-lg bg-slate-900 rounded-xl border border-slate-800 shadow-2xl p-6 flex flex-col gap-4">
            ...
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              ...
              <button
                ...
                className="text-slate-500 hover:text-slate-350 bg-transparent border-0 cursor-pointer text-lg font-bold"
              >
                ✕
              </button>
            </div>

            <div className="p-3 bg-violet-950/40 text-violet-400 border border-violet-900/50 rounded-lg text-xs flex gap-2 items-start leading-relaxed font-semibold">
              <ShieldAlert className="w-4 h-4 shrink-0 text-violet-500" />
              <span>
            ...
            <form onSubmit={handleApprSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-slate-300">결재 제목 (자동 지정)</label>
                <input
                  type="text"
                  value={apprTitle}
                  onChange={(e) => setApprTitle(e.target.value)}
                  className="w-full px-3 py-2 border border-slate-850 rounded-lg text-xs bg-slate-950 text-slate-200 focus:outline-none"
                />
              </div>

              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-slate-300">수정 요청 사유</label>
                <textarea
                  value={apprOpinion}
                  onChange={(e) => setApprOpinion(e.target.value)}
                  placeholder="예: 최신 API 규격 갱신 건으로 기존 문서를 업데이트하고자 상신합니다."
                  rows={4}
                  className="w-full px-3 py-2 border border-slate-850 rounded-lg text-xs bg-slate-950 text-slate-200 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 resize-none"
                  autoFocus
                />
              </div>
