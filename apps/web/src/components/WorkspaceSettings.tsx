...
export default function WorkspaceSettings() {
  ...
  return (
    <div className="flex flex-col gap-8 max-w-3xl w-full mx-auto py-4">
      ...
      {/* 1. 멤버 초대하기 섹션 (OWNER 전용) */}
      {isOwner && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-md flex flex-col gap-4">
          <div className="flex items-center gap-2">
            <UserPlus className="w-5 h-5 text-violet-500" />
            <h3 className="font-bold text-base text-slate-100">동료 초대하기</h3>
          </div>
          ...
          <form onSubmit={handleInvite} className="flex gap-3">
            <input
              ...
              className="flex-1 px-3 py-2 border border-slate-800 rounded-lg text-sm bg-slate-950 text-slate-100 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500"
            />
            <button
              type="submit"
              disabled={inviting}
              className="px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer disabled:bg-violet-600/60 border border-transparent"
            >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-md flex flex-col gap-4">
        <div className="flex justify-between items-center border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-base text-slate-100">소속 팀원 관리</h3>
          </div>
          <span className="text-xs font-semibold text-slate-400">총 {members.length}명</span>
        </div>
            ...
            return (
              <div key={m.userId} className="flex justify-between items-center py-4 first:pt-0 last:pb-0">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-slate-950 rounded-full flex items-center justify-center text-slate-350 font-bold text-sm">
                    {m.user.name.charAt(0)}
                  </div>
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-slate-100">
                      {m.user.name} {isMe && <span className="text-xs text-violet-400 font-medium">(나)</span>}
                    </span>
                    <span className="text-xs text-slate-450 font-mono">{m.user.email}</span>
                  </div>
                </div>
                  ...
                  {isOwner && !isMe && (
                    <div className="flex items-center gap-1.5 ml-2 border-l border-slate-800 pl-3">
                      <button
                        ...
                        className="p-1.5 text-slate-500 hover:text-violet-400 rounded-md hover:bg-slate-800 transition cursor-pointer border border-transparent bg-transparent"
                        title="역할 위임/변경"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-500 hover:text-red-400 rounded-md hover:bg-slate-800 transition cursor-pointer border border-transparent bg-transparent"
                        title="멤버 추방"
                      >
      ...
      {/* 3. 워크스페이스 탈퇴/경고 섹션 */}
      <div className="bg-red-950/10 border border-red-900/30 rounded-xl p-6 shadow-md flex flex-col gap-4">
        <div className="flex items-center gap-2">
          <ShieldAlert className="w-5 h-5 text-red-500" />
          <h3 className="font-bold text-base text-red-400">위험 구역</h3>
        </div>
