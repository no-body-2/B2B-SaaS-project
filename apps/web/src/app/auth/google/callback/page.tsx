...
function GoogleCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      setErrorMsg('Google 로그인 인증이 취소되었거나 거부되었습니다.');
    ...
    <div className="w-full max-w-sm p-8 rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl text-center space-y-4">
      {errorMsg ? (
        ...
        <div className="space-y-4">
          <div className="w-12 h-12 rounded-2xl bg-indigo-950 border border-indigo-800/60 flex items-center justify-center mx-auto text-indigo-400">
            <ShieldCheck className="w-6 h-6 animate-pulse" />
          </div>
          <div>
            <h3 className="text-sm font-bold text-slate-100">Google 계정 인증 중</h3>
            <p className="text-xs text-slate-400 mt-1">
              Google 인증 서버와 토큰을 안전하게 교환하고 있습니다.
            </p>
          </div>
