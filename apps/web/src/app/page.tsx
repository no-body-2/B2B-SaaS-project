...
export default function Home() {
  ...
  const { user, loading, login, register, googleLogin, githubLogin } = useAuth();
  ...
  const handleGitHubLogin = async () => {
    ...
    try {
      ...
      try {
        const configRes = await apiClient.auth.getGitHubConfig();
        if (configRes.data?.clientId) {
          clientId = configRes.data.clientId;
        }
        if (configRes.data?.redirectUri) {
        ...
        console.warn('Failed to fetch dynamic GitHub config, falling back to env.');
      }
      ...
      if (!clientId) {
        await githubLogin('mock-github-authorization-code-1234');
        router.push('/dashboard');
        return;
      }

      const cleanClientId = clientId.trim();
      const cleanRedirectUri = (redirectUri || `${window.location.origin}/auth/github/callback`).trim();
      ...
      console.error(err);
      const parsedMsg = err.response?.data?.message;
      setErrorMsg(parsedMsg || 'GitHub 로그인 처리 도중 에러가 발생했습니다.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-luminano-point rounded-2xl shadow-2xl border border-luminano-border p-8 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            disabled={submitting}
            className="w-full py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-lg text-xs flex items-center justify-center gap-2 transition cursor-pointer border border-slate-700"
          >
